 ...
 * Photon library are just strings of comma seperated numbers with a semicolon
 * at the end 2 = SetAnimation= 2, StripNumber, Animation, Color1, Color2, rate,
 * fade; You can leave off values and defaults will be set for the rest of the
 * values 3 = SetNumberOfLeds = 3, StripNumber, NumberOfLEDsInStrip
 *
 * Color Chart = Red (0..) Orange (32..) Yellow (64..) Green (96..) Aqua (128..)
 * Blue (160..) Purple (192..) Pink(224..) White(255) - This isn't normally on
 * the HUE chart but used in this library to make animaitons easier
 *
 * @author Spectrum3847
 */
public class PhotonLED {
    ...
    private final int kDefaultAnimation = 4;
    private int kDefaultColor1 = 192;
    private int kDeafultColor2 = 255;
    private final int kDefaultRate = 2;
    private final int kDefaultFade = 150;
    ...
    public static enum Animation {
        OFF, SOLID, SOLID_DUAL, BLINK, BLINK_DUAL, SIREN, PULSE_TO_BLACK, PULSE_TO_WHITE, FADE_ALTERNATE, PULSE_DUAL,
        CYLON, CYLON_DUAL, BOUNCE_BAR, BOUNCE_BAR_DUAL, CYLON_MIDDLE, CYLON_MIDDLE_DUAL, TRACER, TRACER_ALTERNATE,
        WIPE_FWD, WIPE_REV, WIPE_FWD_REV, WIPE_DOWN, WIPE_UP_DOWN, WIPE_UP_DOWN_DUAL, WIPE_OUT, WIPE_IN, WIPE_IN_OUT,
        WIPE_IN_OUT_BACK, PERCENTAGE, RAINBOW, JUGGLE, SPARKLES;
    }
    ...
    public PhotonLED(int Color1, int Color2) {
        this();
    ...
    // Set how many LEDs are in one of the strips, call this for each strip that you
    // want to use.
    public void SetNumberOfLEDs(int StripNum, int LEDcount) {
        writeString("3," + StripNum + "," + LEDcount + ";");
    }

    // Set a new animation on a strip, must include a strip number all other values
    // are optional and can be set seperatly.
    public void setAnimation(int StripNum, Integer... vals) {// int Animation, int Color1, int Color2, int rate, int
        // fade){
    ...
    public void setAnimation(int StripNum, Animation a, Color c1, int rate, int fade) { // Allows only a single color to
        // be set
        setAnimation(StripNum, getAnimation(a), getColor(c1), kColor2, rate, fade);
    }

    public void setAnimation(int StripNum, Animation a, Color c1, int rate) { // Allows only a single color to be set
        // but you can adjust the rate
    ...
    // Reset all the values to your default values, useful if you want an animation
    // to use default values, but they may have been already changed.

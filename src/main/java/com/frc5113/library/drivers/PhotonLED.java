...
/**
 * Used to command a Teensy running the photon LED control library. Commands the Photon library are
 * just strings of comma seperated numbers with a semicolon at the end 2 = SetAnimation= 2,
 * StripNumber, Animation, Color1, Color2, rate, fade; You can leave off values and defaults will be
 * set for the rest of the values 3 = SetNumberOfLeds = 3, StripNumber, NumberOfLEDsInStrip
 *
 * <p>Color Chart = Red (0..) Orange (32..) Yellow (64..) Green (96..) Aqua (128..) Blue (160..)
 * Purple (192..) Pink(224..) White(255) - This isn't normally on the HUE chart but used in this
 * library to make animaitons easier
 *
 * @author Spectrum3847
 */
public class PhotonLED {
  ...
  public void setAnimation(
      int StripNum, Integer... vals) { // int Animation, int Color1, int Color2, int rate, int
    // fade){
    ...
    writeString(
        "2," + StripNum + "," + Animation + "," + Color1 + "," + Color2 + "," + Rate + "," + Fade
            + ";");
  }
  ...
  public void setAnimation(
      int StripNum, Animation a, Color c1, int rate, int fade) { // Allows only a single color to
    // be set
    setAnimation(StripNum, getAnimation(a), getColor(c1), kColor2, rate, fade);
  }

  public void setAnimation(
      int StripNum, Animation a, Color c1, int rate) { // Allows only a single color to be set

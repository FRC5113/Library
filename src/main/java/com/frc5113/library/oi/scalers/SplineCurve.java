...
/**
 * This class maps the value of a stick input to a spline curve.
 * 
 * For <code>LINEAR</code> curves, the points provided represent the vertices
 * of a linear spline plotted on a graph (like a line graph).
 * 
 * For <code>SMOOTH</code> curves, the points provided represent the critical
 * points on the graph of the curve. The only exception to this is when
 * <code>points.length</code> is equal to <code>2</code> in which it will always
 * be a linear spline between the two points.
 * 
 * For <code>RIGHT_STEP</code> or <code>LEFT_STEP</code> curves, the points
 * provided represent the points on a step function plotted on a graph. In
 * <code>RIGHT_STEP</code> curves, the value on the interval between points is
 * determined by the value of the rightmost point in the interval. In
...
public class SplineCurve extends Curve {
    ...
    /**
     * Constructs a Spline Curve object which can
     ...
     */
    public SplineCurve(SplineType splineType, double[][] points, double offset, double scalar, double deadzone) {
     ...
     * Constructs a Spline Curve object which can
     * be used to map a stick input based on a custom
     * curve profile. Initialized with default values:
     * <code>
    ...
    public SplineCurve() {
        ...
        points = new double[][]{{-1.0, -1.0}, {0.0, 0.0}, {1.0, 1.0}}; // have to do this to avoid exception handling
        setOffset(0.0);
        setScalar(1.0);
        setDeadDiameter(0.0);
    }
    ...
    private double calculateSplineVal(double input) {
        ...
        if (points.length == 0) {
            ...
            if (input <= points[0][0]) {
                ...
                if (splineType == SplineType.SMOOTH) {
                    if (input <= points[1][0]) {
                        val = calculateSplineSegmentVal(input, SegmentType.LEFT_PARABOLIC, points[0], points[1]);
                    } else if (input >= points[points.length - 2][0]) {
                        val = calculateSplineSegmentVal(input, SegmentType.RIGHT_PARABOLIC, points[points.length - 2], points[points.length - 1]);
                    } else { // input is in central sections
                        boolean valFound = false;
                        int i = 1;
                        while (!valFound && i < points.length - 2) {
                            if (input <= points[i + 1][0]) {
                                val = calculateSplineSegmentVal(input, SegmentType.CUBIC, points[i], points[i + 1]);
                                valFound = true;
                            }
                            i++;
                        }
                    ...
                    // A Linear segment is implied if not one of the others
                    switch (splineType) {
                        case SMOOTH_AT_ENDS:
                    ...
                    while (!valFound && i < points.length - 1) {
                        if (input <= points[i + 1][0]) {
                            val = calculateSplineSegmentVal(input, segmentType, points[i], points[i + 1]);
                            valFound = true;
                        }
                        i++;
                    }
    ...
    private double calculateSplineSegmentVal(double input, SegmentType segmentType, double[] pointA, double[] pointB) {
        switch (segmentType) {
            ...
            case LEFT_PARABOLIC:
                return calculateParabolicSplineVal(input, pointB, pointA); // hack - flip points to change side to left
            case CUBIC:

 ...
 *
 * <p>For <code>LINEAR</code> curves, the points provided represent the vertices of a linear spline
...
public class SplineCurve extends Curve {
  ...
  /**
   * Constructs a Spline Curve object which can be used to map a stick input based on a custom curve
   * profile. Initialized with values provided.
   *
   ...
   */
  public SplineCurve(
      SplineType splineType, double[][] points, double offset, double scalar, double deadzone) {
  ...
  /**
   * Constructs a Spline Curve object which can be used to map a stick input based on a custom curve
   * profile. Initialized with default values: <code>
    ...
    if (points.length == 0) {
      ...
      if (input <= points[0][0]) {
        ...
        if (splineType == SplineType.SMOOTH) {
          if (input <= points[1][0]) {
            val =
                calculateSplineSegmentVal(input, SegmentType.LEFT_PARABOLIC, points[0], points[1]);
  ...
  /**
   * Maps the value of the input to the output using a linear curve between the two points provided.
   *
   * @param input value to be mapped
  ...
  /**
   * The values at each index <code>0</code> in each index of the point array MUST BE IN INCREASING
   * ORDER. There is no code in place to verify this. The array must be a X by 2 array, where X is
   * any valid array length.
   *

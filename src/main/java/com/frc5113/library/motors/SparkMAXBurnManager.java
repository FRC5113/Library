...
/**
 * Determines whether to burn SparkMAX configs to flash. Must be configured with a build date before use, will default to burn if not set
 */
public class SparkMAXBurnManager {
    ...
    public static void update() {
        ...
        if (!file.exists()) {
            ...
            try {
                previousBuildDate =
                        Files.readString(Paths.get(buildDateFile));
            } catch (IOException e) {
                e.printStackTrace();
            }
